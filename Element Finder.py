...
class ElementFinderCommand(sublime_plugin.WindowCommand):
  ...
  # This method is called when the user right-clicks folders in the Side Bar and selects "Find Elements in Folder..."
    ...
    # Ask the user what CSS Selector they want to search for
        ...
        self.window.show_input_panel('Find elements matching CSS selector:',
                                     initial_text, self.on_css_selector_entered, None,
                                     None)
...
'''
                    ...
                    output = json_line['file'] + ' (' \
                        + self.pluralise(json_line['matches'], 'match', 'matches') + ''')

'''
                    match_number = 1
                    for match_detail in json_line['matchesDetails']:
                        output += '    ' + (str(match_detail['line'])
                                + ':').ljust(8, ' ') \
                            + match_detail['html'].replace('\n',
                                '\n        ') + '\n'
        ...
        if self.thread.complete is True:
...
class CommandLineInterface(threading.Thread):
    ...
    # http://docs.python.org/library/subprocess.html#subprocess.Popen
        ...
        try:
            ...
            sublime.error_message('Could not find Node JS at '
                                  + self.settings['node_path']
                                  + '. Check your Element Finder preferences.'
                                  )
            return
